 ...
 * CircularArrayQueue represents a circular array-based implementation of a queue.
...
public class CircularArrayQueue<T> implements QueueADT<T> {
    ...
    /**
     * Constructs a string representation of the elements in the queue.
     * The elements are appended in the order they appear in the queue.
     *
    ...
    public String Message() {
        ...
        for (int i = 0; i < count; i++) {
            sb.append(queue[index]);
            if (i < count - 1) {
                sb.append(" "); // Separate elements with a space
            }
            index = (index + 1) % queue.length;
        }
        return sb.toString();
